...
class HardwareInfoUtil {
    ...
    fun getPhoneSettingsInformation(context : Context) : JSONObject {
        ...
        val alwaysFinishActivities = getPhoneSettingsField(context, Settings.Global.ALWAYS_FINISH_ACTIVITIES)
        val animatorDurationScale = getPhoneSettingsField(context, Settings.Global.ANIMATOR_DURATION_SCALE)
        ...
        val installNonMarketApps = getPhoneSettingsField(context, Settings.Global.INSTALL_NON_MARKET_APPS)
        val modeRinger = getPhoneSettingsField(context, Settings.Global.MODE_RINGER)
        val transitionAnimationScale = getPhoneSettingsField(context, Settings.Global.TRANSITION_ANIMATION_SCALE)

        val usbMassStorageEnabled = getPhoneSettingsField(context, Settings.Global.USB_MASS_STORAGE_ENABLED)
        val wifiNetworksAvailableNotificationOn = getPhoneSettingsField(context, Settings.Global.WIFI_NETWORKS_AVAILABLE_NOTIFICATION_ON)
        ...
        val wifiMaxDhcpRetryCount = getPhoneSettingsField(context, Settings.Global.WIFI_MAX_DHCP_RETRY_COUNT)

        val debugApp = getPhoneSettingsField(context, Settings.Global.DEBUG_APP)
        val wifiMobileDataTransitionWakelockTimeoutMs = getPhoneSettingsField(context, Settings.Global.WIFI_MOBILE_DATA_TRANSITION_WAKELOCK_TIMEOUT_MS)
        val wifiNetworksAvailableRepeatDelay = getPhoneSettingsField(context, Settings.Global.WIFI_NETWORKS_AVAILABLE_REPEAT_DELAY)
        val wifiNumOpenNetworksKept = getPhoneSettingsField(context, Settings.Global.WIFI_NUM_OPEN_NETWORKS_KEPT)
        ...
        information.put("wifi_networks_available_notification_on", wifiNetworksAvailableNotificationOn)
        ...
        information.put("wifi_mobile_data_transition_wake_lock_timeout_ms", wifiMobileDataTransitionWakelockTimeoutMs)
    ...
    fun getHardwareInformation():JSONObject{
            ...
            deviceInformation.put("Build", "Release " + Build.VERSION.RELEASE + ", Inc: '" + Build.VERSION.INCREMENTAL + "'")
    ...
    fun getScreenInformation(windowManager: WindowManager): JSONObject {
