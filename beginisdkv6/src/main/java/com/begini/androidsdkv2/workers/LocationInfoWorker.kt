...
private const val TAG = "LocationInfoWorker"

class LocationInfoWorker(ctx: Context, params: WorkerParameters) : Worker(ctx, params) {
    override fun doWork(): Result {
        return try {
            val appContext = applicationContext
            makeStatusNotification("Location", appContext)
            Timber.d("Starting LocationWorker")
            //Checking if the battery information is requested from the client app.
            val isOptionEnabled = BeginiApp.instance.getBeginiDataSdkOptions().isLocationEnabled
            ...
            if (isOptionEnabled!! && isOptionAllowedInPermission) {
                ...
                Timber.d("Location Worker collection successful")
                //setting result to Begini instance
                BeginiApp.instance.dataSdkResult.LOCATION= ResultConstants.RESULT_SUCCESS
                ...
                if(!isOptionEnabled)
                {
                    BeginiApp.instance.dataSdkResult.LOCATION= ResultConstants.RESULT_FAILED_OPTION_DISABLED
                }else if(isOptionAllowedInPermission){
                    BeginiApp.instance.dataSdkResult.LOCATION= ResultConstants.RESULT_FAILED_PERMISSION_DENIED
                }
            ...
            //setting result to Begini instance
            BeginiApp.instance.dataSdkResult.LOCATION= ResultConstants.RESULT_FAILED_EXCEPTION
            Timber.d("Failure on Location Worker")
    ...
    private fun collectDataSdkItem(): String {
        ...
        val type =
            Types.newParameterizedType(ResultToUploadModel::class.java, LocationModel::class.java)
        val adapter = moshi.adapter<ResultToUploadModel<LocationModel?>>(type)
        val resultFromUtil = BeginiConstants.lastKnownLocation
        val result = ResultToUploadModel<LocationModel?>(
            type = "location",
            integrationId = "",
            uid = "",
        ...
        fileUtil.saveJsonResultToFile(
            ...
            fileName = BeginiConstants.FILE_NAME_LOCATION
        )
        return BeginiConstants.FILE_NAME_LOCATION
    }
}
