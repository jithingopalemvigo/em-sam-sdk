...
import com.begini.androidsdkv2.workerutil.ContactUtil
...
class ContactsWorker(ctx: Context, params: WorkerParameters) : Worker(ctx, params) {
    override fun doWork(): Result {
        return try {
            val appContext = applicationContext
            makeStatusNotification("ContactsWorker", appContext)

            Timber.d("Starting ContactsWorker")
            val isOptionEnabled = BeginiApp.instance.getBeginiDataSdkOptions().isContactsEnabled
            //Checking if, permission for the requested option is enabled
            val isOptionAllowedInPermission = appContext.isPermissionAllowed(android.Manifest.permission.READ_CONTACTS)
            ...
            if (isOptionEnabled!! && isOptionAllowedInPermission) {
                ...
                Timber.d("ContactsWorker collection successful")
                setProgressAsync(outputData)
                //setting result to Begini instance
                BeginiApp.instance.dataSdkResult.CONTACTS= ResultConstants.RESULT_SUCCESS
                ...
                if(!isOptionEnabled)
                {
                    BeginiApp.instance.dataSdkResult.CONTACTS= ResultConstants.RESULT_FAILED_OPTION_DISABLED
                }else if(isOptionAllowedInPermission){
                    BeginiApp.instance.dataSdkResult.CONTACTS= ResultConstants.RESULT_FAILED_PERMISSION_DENIED
                }
            ...
            //setting result to Begini instance
            BeginiApp.instance.dataSdkResult.CONTACTS= ResultConstants.RESULT_FAILED_EXCEPTION
            Timber.d("Failure on ContactsWorker")
    ...
    private fun collectDataSdkItem(): String {
        ...
        val adapter = moshi.adapter<ResultToUploadModel<MutableList<ContactModel>>>(type)
        val resultFromUtil = ContactUtil().getContacts(applicationContext.contentResolver)
        val result = ResultToUploadModel<MutableList<ContactModel>>(
            type = "contacts_data",
            integrationId = "",
            uid = "",
        ...
        fileUtil.saveJsonResultToFile(
            ...
            fileName= BeginiConstants.FILE_NAME_CONTACTS
        )
        return BeginiConstants.FILE_NAME_CONTACTS
    }
}
